...
foreach ($timezoneArray AS $timezone) {
	$timezoneOptions .= "<option ".($settings['default_timezone'] == $timezone ? "selected='selected'" : "").">".$timezone."</option>\n";
}

$date_opts = "<option value=''>".$locale['455']."</option>\n";
foreach($locale['dateformats'] as $dateformat) {
	$date_opts .= "<option value='".$dateformat."'>".showdate($dateformat, time())."</option>\n";
}
unset($dateformat);

$dateTimeZoneGMT = new DateTimeZone("UTC");
$dateTimeZoneDefault = new DateTimeZone($settings['default_timezone']);
//$dateTimeGMT = new DateTime("now", $dateTimeZoneGMT); // not used
$dateTimeDefault = new DateTime("now", $dateTimeZoneDefault);
$timeOffset = $dateTimeZoneGMT->getOffset($dateTimeDefault);

opentable($locale['400']);
echo "<form name='settingsform' method='post' action='".FUSION_SELF.$aidlink."'>\n";
echo "<table cellpadding='0' cellspacing='0' width='500' class='center'>\n<tr>\n";
echo "<td valign='top' width='50%' class='tbl'>".$locale['458']." (".$locale['459']."):</td>\n";
echo "<td width='50%' class='tbl'>".showdate($settings['longdate'], time(), ['tz_override' => $timeOffset])."</td>\n";
echo "</tr><tr>\n";
...
echo "<input type='text' name='shortdate' value='".$settings['shortdate']."' maxlength='50' class='textbox' style='width:180px;' /></td>\n";
echo "</tr>\n<tr>\n";
...
echo "<input type='text' name='longdate' value='".$settings['longdate']."' maxlength='50' class='textbox' style='width:180px;' /></td>\n";
echo "</tr>\n<tr>\n";
...
echo "<input type='text' name='forumdate' value='".$settings['forumdate']."' maxlength='50' class='textbox' style='width:180px;' /></td>\n";
echo "</tr>\n<tr>\n";
...
echo "<input type='text' name='newsdate' value='".$settings['newsdate']."' maxlength='50' class='textbox' style='width:180px;' /></td>\n";
echo "</tr>\n<tr>\n";
...
echo "<input type='text' name='subheaderdate' value='".$settings['subheaderdate']."' maxlength='50' class='textbox' style='width:180px;' /></td>\n";
echo "</tr>\n<tr>\n";
