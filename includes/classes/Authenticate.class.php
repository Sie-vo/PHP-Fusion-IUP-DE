...
class Authenticate {
	...
	public static function setLastVisitCookie() {
		...
		if (iMEMBER) {
			if ($cookie_exists) {
				...
				$update_threads = true;
				$lastvisit = (IsSet($userdata['user_lastvisit']) ? $userdata['user_lastvisit'] : 0);
			}
	...
	private static function _setCookie ($cookieName, $cookieContent, $cookieExpiration, $cookiePath, $cookieDomain, $secure = false, $httpOnly = false) {
		if (version_compare(PHP_VERSION, '5.2.0', '>=')) {
			...
			setcookie($cookieName, (!empty($cookieContent) ? $cookieContent : ""), $cookieExpiration, $cookiePath, $cookieDomain, $secure);
		}
	}
}
